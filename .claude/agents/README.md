...
> **Dernière mise à jour** : 1er juin 2026 (doctrine **auto-amélioration** : chaque agent auto-critique sa propre définition en fin de run — sweep clause standard THI-321 ; déclencheur = 1er run `supabase-backend-auditor` sur le formulaire support). + 29 mai 2026 (doctrine anti-hallucination dénombrement — tous agents : aucun compteur descriptif de tête, source déterministe obligatoire ; suite content-auditor « 63 leçons » faux positif. + 28 mai : `supabase-backend-auditor` Opus, sync modèles, règle zéro-Haiku).
