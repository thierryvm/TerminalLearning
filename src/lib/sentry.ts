...
export function initSentry() {
  ...
  Sentry.init({
    ...
    beforeSend(event: Sentry.ErrorEvent) {
      ...
      // Drop Chrome extension async message channel errors — content scripts
      // injected by browser extensions (password managers, ad-blockers, Honey,
      // 1Password, LastPass, etc.) register `chrome.runtime.onMessage` listeners
      // that race-condition close before `sendResponse()` is called. The error
      // is generated by the extension's runtime, NOT by app code (verified:
      // grep "chrome.runtime|postMessage|MessageChannel" in src/ returns 0 hits).
      // Not actionable from app side. Noise filter only.
      if (
