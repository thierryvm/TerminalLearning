...
> Last updated: 2 May 2026 — **Sprint sécurité 1-2 mai ✅** : audit `security-auditor` frais (score 8.1/10, 0 critical), 3 HIGH résolus — THI-133 LTI feature flag (PR #169), THI-134 LTI cold-start fix (PR #170), THI-135 rate limiter partagé (PR #173). 6 mediums tracés en Linear (THI-136 à 140 + THI-112). H3 git history accepté résiduel. **Phase 7b Security Hardening ✅ complete (THI-120)**: CLAUDE.md credential protection rule + CSP extension + Sentry scrubber double-layer — C1/C2/C3 gate validated ✅ — **Strategic vision consolidated (ADRs 001-005)**: LTI-first positioning, OpenRouter BYOK 4-tiers, TTFR KPI, Classroom Composer UI, AI Tutor V1 decisions — **Epic THI-96 Web 2026 Compliance** 6/8 sub-issues shipped — viewport-fit=cover, min-h-dvh, safe-area-inset-bottom, WCAG 2.2 AAA touch targets, focus-visible rings, clamp() typography — **shadcn/ui migration ✅** (THI-91 umbrella) — INP fix THI-90 ✅, lab 515ms → 26ms (−95%) — Phase 4c Bundle Optimization ✅ — 11 modules / 65 lessons / 1000+ tests — **Next**: Phase 7c LTI activation (RS256 JWK + persistence + grade passback), THI-111 AiTutorPanel, THI-112 onboarding, dashboards par rôle (étudiant/prof/formateur/institution/super-admin), i18n FR/NL/EN/DE
