...
> Dernière mise à jour : 5 mai 2026 soirée UTC+1 — Phase 7c THI-152 sprint mobile recovery EN COURS (8/9 mini-PRs + hotfix 7bis livrées : focus traps a11y + forms anti-zoom + FAB Sparkles size/opacity/position + PWA apple-touch-icon PNG + touch targets ≥44/≤40 + drawer overflow word-break + PWA safe-area top + autoFocus terminal contrôlé + hotfix 7bis Landing nav safe-area + **focus rings emerald harmonization** ; reste 1 mini-PR : theme-color media + W3C mobile-web-app-capable (9/9)). **Brick 8/9** : 8 occurrences `outline` harmonisées vers pattern canonique TL `outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/60 focus-visible:ring-offset-0` (AiTutorPanel ×5, MessageInput ×2, RateLimitBadge ×1). Voie safe @cowork respectée : aucune modif `ui/button.tsx` ni `ui/input.tsx` shadcn (patterns `--ring` CSS var préservés). 6 patterns FLAGUÉS intentionnels (LoginModal hybride, UserMenu logout rouge, UserMenu avatar full opacity, switch-mode link `ring-0`, shadcn defaults). THI-150 ✅ + THI-151 ✅ + 5 sub-PRs THI-152 (#196/#197/#198/#199/#200 en cours) toutes mergées sauf #200. Empirical override mini-PR 3/9 confirmé par @thierry (FAB mobile 48→44 px, desktop 56 inchangé, asymétrie intentionnelle primary action exemption).
