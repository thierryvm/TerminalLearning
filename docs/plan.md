...
> Dernière mise à jour : 4 mai 2026 18:00 UTC+1
> Statut global : **Phase 5 EN COURS** — Curriculum Expansion : 11 modules ✅, 65 leçons, **1267 tests unitaires** (1247 pass + 20 RBAC skipped Phase 9) + 176 E2E — **Vision consolidée** : LTI-first (ADR-001), BYOK OpenRouter 4-tiers (ADR-002), TTFR KPI central (ADR-003), Classroom Composer UI (ADR-004), AI Tutor V1 décisions gelées (ADR-005 — stockage, rate-limit, guardrails), Solo-sustainable practices (ADR-006), tuteur IA socratique dès A1, i18n FR/NL/EN — Architecture stratégique précédente (THI-35) : Terminal Sentinel (Phase 5.5) ✅, RBAC complet (Phase 7) ✅, Admin Panel (Phase 9), PWA avancée (Phase finale) — **Epic Web 2026 Compliance** (THI-96) : 6/8 sub-issues livrées (THI-97 → THI-102), reste Desktop a11y + CSS moderne 2026 — **Phase 7b (AI Tutor V1) ✅ COMPLETE** : THI-115 ✅, THI-109 ✅ (gate-zero guardrail), THI-110 ✅ (keyManager AES-GCM), THI-120 ✅ (Sentry scrubber), **THI-111 ✅ COEUR FONCTIONNEL** (PR #188 — sanitizer + 4 providers + panel + 287 AI tests, audits guardrail 9.4/10 + security 8.8/10 + ui A11y exemplary, scope mount lesson+dashboard+reference, FAB Sparkles, consent checkbox RGPD, validation live 3/4 providers — OpenAI bloqué CORS officiel, disclaimer ajouté + recommandation OpenRouter pour modèles GPT) — **Sprint sécurité 1-2 mai 2026 ✅ CLOS** : audit security-auditor 8.1/10 → ~8.6/10 post-sprint, 11 PRs livrées (#168 à #178), 5 HIGH/MEDIUM Done (THI-133/134/135/137/140), 4 MEDIUM ciblés en backlog (THI-136/138/139/112), agent `route-attack-auditor` créé pour combler la zone HTTP-level — **Session 4 mai 2026** : THI-111 livré (13 commits, 3 audits release-ready, validation Chrome DevTools MCP autonome confirmant Anthropic + Gemini + sanitizer reject + format mismatch + mobile iPhone 14 Pro + Escape + a11y), test live OpenRouter en cours par Thierry sur preview Vercel après activation `VITE_AI_TUTOR_ENABLED=true` (kill-switch documenté dans `docs/processes/feature-flags.md`) — **Posture validée** : pas de rush deadline, qualité/scalabilité/perf non négociables, plan respecté en ordre — **Next** : THI-112 V1.5 (AiKeySetup tutoriel visuel + picker modèle curated allow-list + mode chiffré opt-in UI + AiConsentModal version log) → THI-113 (audit final post-ship réel) → THI-114 (Web Worker isolation V1.5 — clé jamais main thread)
