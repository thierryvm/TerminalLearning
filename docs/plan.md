...
> Dernière mise à jour : 2 mai 2026 17:30 UTC+1
> Statut global : **Phase 5 EN COURS** — Curriculum Expansion : 11 modules ✅, 65 leçons, 1035 tests unitaires (1015 pass + 20 RBAC skipped Phase 9) + 176 E2E — **Vision consolidée** : LTI-first (ADR-001), BYOK OpenRouter 4-tiers (ADR-002), TTFR KPI central (ADR-003), Classroom Composer UI (ADR-004), AI Tutor V1 décisions gelées (ADR-005 — stockage, rate-limit, guardrails), Solo-sustainable practices (ADR-006), tuteur IA socratique dès A1, i18n FR/NL/EN — Architecture stratégique précédente (THI-35) : Terminal Sentinel (Phase 5.5) ✅, RBAC complet (Phase 7) ✅, Admin Panel (Phase 9), PWA avancée (Phase finale) — **Epic Web 2026 Compliance** (THI-96) : 6/8 sub-issues livrées (THI-97 → THI-102), reste Desktop a11y + CSS moderne 2026 — **Phase 7b (AI Tutor V1) ✅ COMPLETE pour les briques sécurité** : THI-115 ✅, THI-109 ✅, THI-110 ✅, THI-120 ✅ (C1/C2/C3 security) — **Sprint sécurité 1-2 mai 2026 ✅ CLOS** : audit security-auditor frais 8.1/10 → ~8.6/10 post-sprint, 11 PRs livrées (#168 à #178), 5 HIGH/MEDIUM Done (THI-133/134/135/137/140), 4 MEDIUM ciblés en backlog (THI-136/138/139/112), agent `route-attack-auditor` créé pour combler la zone HTTP-level — **Session 2 mai après-midi** : 3 PRs cleanup (#180 règle `gh pr list` shutdown, #149 THI-108 leçon merge-strategies +6 tests Sourcery flexible regex, #150 agents-depth Sourcery cleanup), gate-zero `prompt-guardrail-auditor` ✅ CLEAN avant THI-111, plan détaillé écrit dans `.claude/plans/thi-111-aitutorpanel.md`, 4 décisions Q&A actées (feature flag opt-in, modèle `llama-3.3-70b-instruct:free` par défaut, trigger Ctrl+I + icône bas-droit, ton socratique adaptatif anti-frustration) — **Posture validée** : pas de rush deadline, qualité/scalabilité/perf non négociables, plan respecté en ordre — **Next**: THI-111 (sanitizer.ts FIRST → systemPrompt → 4 providers → useAiTutor hook → AiTutorPanel) → THI-112 (onboarding AiKeySetup + AiConsentModal + provider picker complet) → THI-113 (audit final security-auditor + prompt-guardrail-auditor)
