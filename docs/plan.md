...
| THI-91 | tech-debt | **Umbrella shadcn migration** — seule source de vérité pour le scope restant. Landing A/B/C ✅ (PR #118, #124, #125) + Dashboard ✅ (enfant **THI-95**, PR #131, 17 avril 2026) + MarkdownPage (ChangelogPage / StoryPage) 🔄 PR #134 + PWAInstallModal + CommandReference 🔄 PR #135 + Sidebar + MenuButton 🔄 PR #136 (stacked on #135) + LessonPage consolidation 🔄 PR #137 — 4 PRs ouvertes le 17 avril 2026, en attente validation visuelle Thierry. Variantes ajoutées : `tl-icon-ghost` / `tl-tab` / `tl-tab-active` / `tl-filter-pill` / `tl-filter-pill-active` / `tl-sidebar-icon` / `tl-sidebar-row` / `tl-sidebar-row-locked` / `tl-sidebar-lesson` / `tl-env-pill` / `tl-menu-fab`. Sizes ajoutés : `tl-icon-sm` / `cta-pill-sm` / `tl-install-cta` / `tl-tab-size` / `tl-filter-pill-size` / `tl-icon-44` / `tl-icon-44-md` / `tl-sidebar-row` / `tl-sidebar-lesson` / `tl-env-pill` / `tl-nav-inline` / `tl-nav-inline-xs` / `tl-nav-cta`. `nav-link` / `ghost-gh` / `emerald-soft` enrichis avec le focus-visible emerald ring pour cohérence globale. 🔄 In Review |
