...
| THI-85 | tech-debt | Migrer composants custom vers shadcn/ui — page par page. NotFound.tsx ✅ (PR #116, 14 avril 2026 — variantes `emerald` / `ghost-gh` / `pill-*`). Enhance 404 (PR #117, 14 avril 2026) — bloc "Pages utiles" SEO/GEO crawlable + fix contraste footer iPhone 14 (`#484f58` → `#8b949e`). Reste : LessonPage, ChangelogPage, StoryPage, CommandReference 🔄 In Progress |
| THI-91 | tech-debt | shadcn migration umbrella — Landing A/B/C ✅ (PR #118, #124, #125) + Dashboard ✅ (PR #131, 17 avril 2026). Reste : LessonPage, ChangelogPage, StoryPage, CommandReference 🔄 In Progress |
| THI-95 | tech-debt | Dashboard.tsx — migration shadcn/ui complète (3 variantes Card `tl-surface`/`tl-stat`/`tl-module`, 2 variantes Progress `tl`/`tl-thin`, 1 variante Button `tl-ghost` + size `tl-list-row`, CSS var `--tl-progress-color` pour couleur par module) (PR #131, 17 avril 2026) ✅ Done |
